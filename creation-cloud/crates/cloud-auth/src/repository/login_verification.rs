...
use super::error;
use crate::verification::{MAX_ATTEMPTS, RESEND_COOLDOWN_SECONDS};
...
pub(crate) async fn replace_open(
    ...
    if last_delivery_started_at.is_some_and(|started_at| {
        Utc::now().signed_duration_since(started_at).num_seconds() < RESEND_COOLDOWN_SECONDS
    }) {
        return Err(AppError::RateLimitedAfter {
            message: "登录验证码发送过于频繁，请稍后重试".to_owned(),
            retry_after_seconds: retry_after_seconds(last_delivery_started_at),
        });
    }
    sqlx::query(
...
fn retry_after_seconds(last_delivery_started_at: Option<DateTime<Utc>>) -> u64 {
