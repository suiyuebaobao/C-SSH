...
pub(crate) async fn execute(
    ...
    if !password_valid {
        ...
        if let Some(error) =
            super::login_lockout::record_failed_password(pool, &account, &identifier).await?
        {
            return Err(error);
        }
    ...
    super::login_lockout::ensure_not_locked(&current_account, Utc::now())?;
    ...
    repository::login::clear_login_failures(&mut transaction, current_account.id).await?;
    ...
    Ok(LoginOutcome::Session(Box::new(IssuedSession {
        ...
        metadata: crate::session::SessionMetadata::unbound_with_request(
            expires_at,
...
fn captcha_required_for_purpose(
    ...
    match purpose {
        ...
        CaptchaPurpose::Register | CaptchaPurpose::Login | CaptchaPurpose::PasswordReset => {
            user_captcha_enabled
        }
    }
}
